...
Step 1 estimates the empirical relationship between local weather anomalies and socioeconomic outcomes, such as household consumption, income, and poverty status. Models are specified and fitted to historical data to quantify how weather shocks affect welfare on average and across population sub-groups, forming the statistical foundation for the simulations in [Step 2 (Climate Scenarios)](step2.qmd) and [Step 3 (Policy Scenarios)](step3.qmd).
...
- **Characteristics tables:** summary statistics for individual-, household-, firm-, and area-level variables (filtered to the selected level of analysis).
- **Selected surveys:** the survey files backing the current sample.
...
The interview-timing chart and the location map together determine how much exploitable weather variation the sample contains: the map shows *where* interviews are spread, the timing chart shows *when*. Interviews concentrated in a single season or a handful of locations leave little within-location variation for the fixed-effects model to identify from.

- Use the **outcome stats** table to check missingness in candidate outcomes before selecting one — high missingness limits the effective sample once covariates are introduced in Step 4.
...
- **Continuous Outcomes:** Household consumption (`welfare`) or other continuous welfare metrics per capita per day, as flagged in the variable metadata.
...
- **Currency:** For monetary aggregates, choose between:
  ...
  - **LCU (2021)** — local currency units. LCU outcomes are converted back to constant 2021 PPP dollars before estimation, so results remain comparable across economies; welfare is additionally bottom-coded at \$0.28/day at load.
...
- The coverage map identifies where outcome data is missing. Coverage gaps translate directly into sample loss once covariates with missing values enter the model in Step 4.
...
WISE-APP applies a logarithmic transformation to all continuous (numeric) outcomes during estimation, so coefficients are interpreted on the log scale. Ensure raw consumption values are strictly positive and not pre-log-transformed in the source data.
:::
...
- **Variable Treatment:**
  - *Binned* (default): Converts the weather variable into categorical bins using a **Number of bins** slider (2–10, default 5). Binned specifications do not impose parametric functional forms and capture non-linear effects. Binning methods:
    - `Equal frequency` (sample quantiles, default)
    - `Equal width`
    - `K-means` clustering
    - `Custom` comma-separated break points — expect N−1 interior cuts for N bins; the outermost bins are open-ended so out-of-range values still map to the extreme bins. Duplicate bins are dropped, which can reduce the bin count below the requested number.
    Bin breaks are derived from the distribution of the configured weather value across the **survey sample**, not from the historical climate.
...
- **Distribution of weather:** each wave's distribution with that wave's own climate history (over the configured year range) drawn alongside it.
...
The available model types depend on the outcome selected in Step 2:
...
- **Quantile Regression (RIF):** Fits unconditional quantile regressions via Recentered Influence Functions, estimating how weather impacts specific quantiles ($\tau = 0.1, 0.2, \dots, 0.9$) across the welfare distribution. Offered when a continuous outcome is selected.
...
The interaction coefficient $\gamma$ captures differential weather sensitivity between households with and without access as observed in the survey. Step 3 simulates policies by assigning non-access households the access group's sensitivity — an assumption that the sensitivity gap is caused by the access variable itself rather than by unobserved characteristics correlated with it (see the identification caveat in [Methodology in detail](#methods)). Treat simulated policy impacts as scenario arithmetic, not causal treatment effects.
:::
...
- **Covariate Selection:**
  ...
  - **LASSO Stability Selection:** Automated selection via `glmnet` with 10-fold cross-validation (`lambda.1se` by default; an advanced panel exposes the elastic-net mixing parameter, fold count, standardization, and lambda rule). The selection is repeated 5 times with varying cross-validation folds — or across 5 MICE multiple imputations if *Use MICE imputation* is enabled (off by default; without it, complete cases are used) — and candidate variables selected in at least 50% of iterations (adjustable stability threshold) are retained in the final model. Forced inclusion and exclusion lists are supported. Note that all model variables must be at least 90% complete within each survey round, so variables with more than 10% missing values never enter the candidate pool.
...
2. **Progressive Coefficient Plot:** Estimated weather and interaction coefficients with 95% confidence intervals across three nested specifications:
   ...
   - *Model 2 (FE):* Adds fixed effects and the moderator main effects.
...
- Scale reminder: for continuous outcomes estimated in logs, coefficients are semi-elasticities; for logistic regression they are log-odds ratios (see [Methodology in detail](#methods)).
...
- **Residuals vs. weather** should look patternless. Residual U-shapes or trends in the tails indicate a mis-specified functional form — revisit the binned/continuous choice in Step 3.
...
- $\varepsilon_{ijt}$ is the idiosyncratic error term. Standard errors are cluster-robust at the survey-location panel level ($\texttt{loc\_id\_panel}$), the level at which the weather hazard is assigned^[See [@moulton1990], [@cameronmiller2015], and [@abadie2023].], and the same VCV specification is reused when coefficient uncertainty is propagated in Step 2. Survey weights are not applied in Step 1 estimation (see the *Survey Design & Standard Errors* note above).
...
Three nested specifications of this equation are estimated on the same sample — (1) weather and interaction terms only, (2) adding fixed effects and moderator main effects, and (3) the full model with covariates — producing the progressive comparison in the Results tab.
...
The $\text{Haz}_{kt}$ term enters the regression under one of two parameterizations, chosen per weather variable in Step 3:
...
Because bin effects make no functional-form assumption, comparing the binned fit against a continuous specification (and across bin counts) is the recommended robustness check; the binscatter diagnostics in Step 3 preview which parameterization the data supports.
