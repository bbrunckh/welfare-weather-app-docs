...
- **Metadata** files are small CSV files that catalog survey rounds, variables, and monetary conversions. They are loaded when WISE-APP initializes and drive the user interface.
...
The pipelines used to prepare datasets for WISE-APP Online are maintained in the [welfare-weather-app-data](https://github.com/worldbank/welfare-weather-app-data){target="_blank"} repository, along with guidance to prepare other data sources.
:::

### Local R Package

When running the application locally in R, the **Overview** landing page provides an interactive **Data** panel to connect to data in a local folder or remote file system. Supported remote storage includes Databricks, AWS S3, Google Cloud Storage, Hugging Face, and Azure. 
...
WISE-APP links microdata and hazard layers across space using the [H3 spatial indexing system](https://h3geo.org)[^h3], which partitions the globe into hexagonal cells.
...
::: {.callout-tip appearance="simple"}
### Custom data
When running WISE-APP locally, users can bring their own microdata (individual, household or firm level) or hazard data provided that it follows the standardized [WISE-APP data schema](#wise-app-data-schema) and directory structure.
:::

### Household Survey Microdata (GMD)

Household survey microdata are from the World Bank’s **Global Monitoring Database (GMD)**[^gmd], a harmonized collection of nationally representative multi-topic household surveys covering over 150 low- and middle-income countries across more than 2,000 survey waves from 1989 to the present. GMD provides standardized welfare aggregates, poverty measures, and demographic indicators. 

While many efforts are made to harmonize survey variables, some variables are only available in a subset of surveys. In addition, variables may not be strictly comparable across surveys due to differences in survey design, question wording, and sampling frames.

<!-- Would like to move this to relevant location in analysis steps
::: {.callout-warning appearance="simple"}
### Survey Design & Variance Estimation
GMD does not harmonize complex survey design parameters (such as primary sampling units, strata, and clusters). Standard errors are cluster-robust at the location level (`loc_id`), but may understate true sampling variance if multi-stage sampling effects are strong.
:::  -->
...
[Geocoding GMD documentation](https://datanalytics-int.worldbank.org/geocoding-gmd/){target="_blank"} explains how GMD surveys are geocoded to H3 hexagons using available location information (GPS coordinates or mappable regions).
:::
...
GMD surveys exhibit substantial variability in temporal and geographic precision. Some surveys conduct interviews rolling across all 12 calendar months with granular GPS coordinates representing thousands of distinct clusters. Others are conducted over only a few months or are spatially anonymized to coarse administrative units (e.g., province or region). 
...
Historical weather data are derived from **ERA5-Land**[^era5-land], a global climate reanalysis produced by the European Centre for Medium-Range Weather Forecasts (ECMWF). ERA5-Land combines physical modeling with global in-situ and satellite observations to provide consistent, hourly estimates of meteorological variables at a ~9 km spatial resolution from 1950 to the present.

WISE-APP uses monthly aggregates of selected ERA5-Land variables (e.g., mean maximum daily temperature, number of very hot days, precipitation sums) sourced from the dataset underpinning the Copernicus Interactive Climate Atlas[^c3s-atlas], spatially mapped onto H3 hexagonal cells for linking to microdata. The full list of weather variables available is provided in the **Variable list** section below (filter to `hazard`).

### Climate Change Scenarios (CMIP6 SSPs)

For climate projections, WISE-APP uses data derived from the **Coupled Model Intercomparison Project Phase 6 (CMIP6)**[^cmip6] representing three Shared Socioeconomic Pathways (SSPs):
...
WISE-APP uses CMIP6 data for the same weather variables derived from ERA5-Land for up to 30 ensemble members at native grid resolutions (typically 100–250 km), for both baseline (1950–2014) and projection periods (2015–2100). These are sourced from the same Copernicus Interactive Climate Atlas dataset[^c3s-atlas]. Following best practice, WISE-APP uses the full ensemble rather than a single or median GCM to capture uncertainty in climate projections[^burke2015]. This approach recognizes that adaptation and resilience policies should be informed by the full range of possible climate outcomes as relying only on central estimates can be misleading.
...
The table below lists all variables in GMD microdata and ERA5-Land / CMIP6 hazard data prepared for the [internal World Bank web app](https://datanalytics-int.worldbank.org/wise-app){target="_blank"}. It also defines their role in the application. This is encoded by the `variable_list.csv` metadata file, which is loaded when WISE-APP initializes. 

::: {.callout-warning appearance="simple"}
### Variable Coverage & Missing Data
Not all variables are available in every survey round and some are only available for specific levels of analysis. Check the **Survey stats** tab to review variable availability for the selected sample.
:::
...
- Optional column: `pop_2020` (GHS-POP 2020 population count[^ghs-pop]), used to weight weather aggregations when a survey cluster spans multiple H3 cells.
...
Modules for discrete historical and probabilistic extreme events (e.g., riverine floods, tropical cyclone return periods) are planned for future releases of WISE-APP. See [welfare-weather-app-data](https://github.com/worldbank/welfare-weather-app-data){target="_blank"} 
:::

[^h3]: Uber Technologies (2018). H3: A Hexagonal Hierarchical Spatial Index. <https://h3geo.org>.

[^gmd]: World Bank (2026). Global Monitoring Database (GMD). World Bank, Washington, DC.

[^era5-land]: Muñoz-Sabater, J., et al. (2021). ERA5-Land: A state-of-the-art global reanalysis dataset for land applications. *Earth System Science Data*, 13(9), 4349–4383. DOI: [10.5194/essd-13-4349-2021](https://doi.org/10.5194/essd-13-4349-2021).

[^c3s-atlas]: Copernicus Climate Change Service, Climate Data Store (2024). Gridded dataset underpinning the Copernicus Interactive Climate Atlas. Copernicus Climate Change Service (C3S) Climate Data Store (CDS). DOI: [10.24381/cds.h35hb680](https://doi.org/10.24381/cds.h35hb680).

[^cmip6]: Eyring, V., et al. (2016). Overview of the Coupled Model Intercomparison Project Phase 6 (CMIP6) experimental design and organization. *Geoscientific Model Development*, 9(5), 1937–1958. DOI: [10.5194/gmd-9-1937-2016](https://doi.org/10.5194/gmd-9-1937-2016).

[^burke2015]: Burke, M., Dykema, J., Lobell, D. B., Miguel, E., & Satyanath, S. (2015). Incorporating climate uncertainty into estimates of climate change impacts. *Review of Economics and Statistics*, 97(2), 461–477. DOI: [10.1162/REST_a_00478](https://doi.org/10.1162/REST_a_00478).

[^ghs-pop]: Schiavina, M., Freire, S., & MacManus, K. (2022). GHS-POP R2022A - GHS population grid multitemporal (1975–2030). European Commission, Joint Research Centre (JRC). DOI: [10.2905/D83C0D50-00A9-4A23-9023-EFF10426AA37](https://doi.org/10.2905/D83C0D50-00A9-4A23-9023-EFF10426AA37).
